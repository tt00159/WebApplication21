$(function () {
                ...
                if (json.length > 0) {
                    for (i = 0; i < json[0].HSCodeRate.length; i++) {
                        ...
                        for (j = 0; j < json[0].HSCodeRate[i].Materials.length; j++) {
                            ...
                            tr += "<td class='center'>" + json[0].HSCodeRate[i].Materials[j].EBELN + "</td>";
                            ...
                            tr += "<td class='text-right'>" + json[0].HSCodeRate[i].Materials[j].ItemTariffMargin.toFixed(2) + "</td>";
                            tr += "<td class='text-right'>" + json[0].HSCodeRate[i].Materials[j].ItemTariffOtherCost.toFixed(2) + "</td>";
                            tr += "<td class='text-right'>" + json[0].HSCodeRate[i].Materials[j].ItemTariffAmt.toFixed(2) + "</td>";
                            tr += "</tr>";
                    ...
                    $("#lblTariffMargin").text(json[0].TariffMargin.toLocaleString());
                    $("#lblTariffOtherCost").text(json[0].TariffOtherCost.toLocaleString());
    ...
    if (vSystemNo != undefined && vSystemNo.length > 0) {
        ...
        $(menu).attr("url", "/Report/ImportTariff");
        $("#btnSubmit").click();
    }

    $("#btnPrint").click(function () {
        window.open('/Static/PrintData.html', '進貨付稅', config = 'left=0,top=0,height=500,width=1000,toolbar=no,scrollbars=no,resizable=no,location=no,menubar=no,status=no');
    });
});
