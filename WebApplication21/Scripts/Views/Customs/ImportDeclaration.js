$(function () {
	...
	$("#formHandler").submit(function (e) {
		...
		$.ajax({
			...
			success: function (json) {
				CloseBlockUI();
				AlertResultMsg("formHandler", json, "/Customs/ImportDeclaration");
			}
		});
	});
		...
		bootbox.confirm({
			...
			callback: function (result) {
				if (result) {
					...
					$.ajax({
					    url: "/Customs/ImportDeclarationHandler",
						type: 'post',
						dataType: 'json',
	...
	$("#jqMaterial").click(function () {
		if (!$(this).hasClass("disabled")) {
			...
			$(menu).attr("url", "/Customs/DeclarationMaterial?TradeKind=Import&DeclarationNo=" + rowObject['DeclarationNo']);
    ...
    if (BUKRS != undefined && DeclarationNo != undefined) {
		...
		$.ajax({
			...
			success: function (json) {
				...
				$('#txtConsigneeNo').val(json.ConsigneeNo);
				$('#txtConsignee').val(json.Consignee);
				$('#txtImportPortNo').val(json.ImportPortNo);
				$('#txtImportPort').val(json.ImportPort);
				$('#txtRecordNo').val(json.RecordNo);
				$('#txtImportDate').val(json.ImportDate);
				$('#txtApplicationDate').val(json.ApplicationDate);
				$('#txtConsumeUsedNo').val(json.ConsumeUsedNo);
				$('#txtConsumeUsed').val(json.ConsumeUsed);
				...
				$('#txtOriginCountryNo').val(json.OriginCountryNo);
				$('#txtOriginCountry').val(json.OriginCountry);
				$('#txtLoadingPortNo').val(json.LoadingPortNo);
				$('#txtLoadingPort').val(json.LoadingPort);
				$('#txtDestinationNo').val(json.DestinationNo);
